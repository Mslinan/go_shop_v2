<template>
  <div class="shop-info" ref="infoRef">
    <div class="info-content">
      ...
      <section class="section">
        ...
        <div class="activity">
          <div class="activity-item" :class="classColor[item.type]" v-for="(item, index) in info.supports" :key="index">
...
export default {
  ...
  methods: {
    initScroll () {
      new BScroll (this.$refs.infoRef, {
        click: true,
      })
      // 给ul指定样式宽度
      const ul = this.$refs.picRef
      const liWidth = 120
      const space = 6
      const liCount = this.info.pics.length
      const width = (liWidth + space) * liCount - space
      ul.style.width = width + 'px'
      new BScroll (this.$refs.picRef, {
        click: true,
        scrollX: true,
      })
    }
