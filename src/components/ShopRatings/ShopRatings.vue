<template>
  <div class="ratings" ref="ratingRef">
    <div class="ratings-content">
      ...
      <RatingsFilter :type="type" :isOn="isOn"></RatingsFilter>
      <!-- 评论列表 -->
      <div class="rating-wrapper">
        <ul>
          <li class="rating-item" v-for="(rating, index) in setType" :key="index">
            <div class="avatar">
...
export default {
  data() {
    return {
      type: 2,  // 指定显示的评论类型  2: 全部, 0: 推荐, 1: 吐槽
      isOn: true, // 是否显示没有内容的评论
    }
  },
  computed: {
    ...mapState({
    ...
    // 构造指定类型数组评论
    setType () {
      const {ratings,type,isOn} = this
      return ratings.filter(rating => {
        const {rateType, text} = rating
          return ((type===2 || type === rateType) && (text.length > 0 || !isOn))
        })
      }
  },
  mounted () {
    this.$eventBus.$on('setSelectType', this.setSelectType)
    this.$eventBus.$on('seeOnly', this.seeOnly)
  ...
  watch: {
    // 解决刚刚进入页面，使用滚动
    ratings () {
      // 监听ratings得到以后，调用initScroll函数，创建滚动对象
      this.$nextTick(() => {
        this.initScroll()
      })
    }
